...
class ProcessManager:
    ...
    def _create_widgets(self):
        self.system_stats_label = ttk.Label(
            self.root, text="Загрузка системы: CPU 0%, RAM 0%", anchor="center", style="TLabel"
        )
    ...
    def _create_top_frame(self):
        ...
        ttk.Button(top_frame, text="Выбрать .exe", command=self._select_exe).pack(side=tk.LEFT, padx=5)
        ttk.Button(top_frame, text="Запустить окно", command=self._launch_game).pack(
    ...
    def _create_main_frame(self):
        ...
        self.tree = ttk.Treeview(main_frame, columns=("hwnd", "ram"), show="headings", height=15)
    ...
    def _create_side_controls(self, parent):
        ...
        self.single_resize_combo = ttk.Combobox(side_controls, values=["800x600", "640x480", "320x240"], width=10)
        self.single_resize_combo.set("800x600")
        self.single_resize_combo.pack(pady=5)

        ttk.Button(side_controls, text="Применить", command=self._apply_resize).pack(fill="x", pady=2)
        ttk.Button(side_controls, text="Закрыть окно", command=self._apply_close).pack(fill="x", pady=2)
        ttk.Button(side_controls, text="Закрыть все окна", command=self._close_all_windows).pack(fill="x", pady=2)
    ...
    def _create_autoclicker_controls(self, parent):
        ...
        self.btn_start_clicker = ttk.Button(parent, text="Старт", command=self._start_autoclicker)
        self.btn_start_clicker.pack(fill="x", pady=2)

        self.btn_select_area = ttk.Button(
            parent, text="Выбрать область", command=lambda: self.autoclicker.select_area(self.root)
        )
        self.btn_select_area.pack(fill="x", pady=2)

        self.btn_stop_clicker = ttk.Button(parent, text="Стоп", command=self._stop_autoclicker)
        self.btn_stop_clicker.pack(fill="x", pady=2)

        ttk.Label(parent, text="Тайминги (сек)").pack(anchor="w", pady=(10, 0))

        self.sleep_found_var = tk.DoubleVar(value=self.autoclicker.sleep_found)
        self.sleep_not_found_var = tk.DoubleVar(value=self.autoclicker.sleep_not_found)

        row1 = ttk.Frame(parent, style="Dark.TFrame")
        row1.pack(fill="x", pady=2)
        ttk.Label(row1, text="После клика:").pack(side=tk.LEFT)
        self.sb_sleep_found = ttk.Spinbox(
            row1, from_=0.0, to=5.0, increment=0.01, textvariable=self.sleep_found_var, width=8
        )
        self.sb_sleep_found.pack(side=tk.RIGHT)

        row2 = ttk.Frame(parent, style="Dark.TFrame")
        row2.pack(fill="x", pady=2)
        ttk.Label(row2, text="Если не найдено:").pack(side=tk.LEFT)
        self.sb_sleep_not_found = ttk.Spinbox(
            row2, from_=0.0, to=5.0, increment=0.01, textvariable=self.sleep_not_found_var, width=8
        )
        self.sb_sleep_not_found.pack(side=tk.RIGHT)

        self._set_clicker_controls_state(running=False)

    def _set_clicker_controls_state(self, running: bool):
        if running:
            self.sb_sleep_found.configure(state="disabled")
            self.sb_sleep_not_found.configure(state="disabled")
            self.btn_select_area.configure(state="disabled")
            self.btn_start_clicker.configure(state="disabled")
            self.btn_stop_clicker.configure(state="normal")
        else:
            self.sb_sleep_found.configure(state="normal")
            self.sb_sleep_not_found.configure(state="normal")
            self.btn_select_area.configure(state="normal")
            self.btn_start_clicker.configure(state="normal")
            self.btn_stop_clicker.configure(state="normal")
    ...
    def _create_bottom_frame(self):
        ...
        self.resize_all_combo = ttk.Combobox(left_frame, values=["800x600", "640x480", "320x240"], width=10)
        self.resize_all_combo.set("800x600")
        self.resize_all_combo.pack(side=tk.LEFT, padx=5)

        ttk.Button(left_frame, text="Применить ко всем", command=self._resize_all).pack(side=tk.LEFT, padx=10)

        link_label = tk.Label(
            right_frame, text="t.me/doomsday_script", fg="skyblue", cursor="hand2", bg="#1e1e1e"
        )
        link_label.pack(side=tk.RIGHT)
        link_label.bind("<Button-1>", lambda e: os.system("start https://t.me/doomsday_script"))
    ...
    def _get_doomsday_windows(self):
        ...
        def enum_callback(hwnd, _):
            try:
                ...
                if proc.name().lower() == "doomsday.exe" and win32gui.IsWindowVisible(hwnd):
                    hwnds.append(hwnd)
            except Exception:
                pass
    ...
    def _start_autoclicker(self):
        try:
            found = float(self.sleep_found_var.get())
            not_found = float(self.sleep_not_found_var.get())
            if not_found < 0.005:
                not_found = 0.005
            self.autoclicker.set_sleeps(found=found, not_found=not_found)
        except Exception:
            pass

        if self.autoclicker.start():
            self.ruchki_status.config(text="Статус: В РАБОТЕ", foreground="lime")
            self._set_clicker_controls_state(running=True)
    ...
    def _update_system_stats(self):
        ...
        self.system_stats_label.config(text=f"Загрузка системы: CPU {cpu:.0f}%, RAM {ram:.0f}%")
        self._update_table()
